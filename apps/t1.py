...
#			'Angel Beats')
	...
	def test_add_serie(self):
		...
		c = Client()
		response = c.post('/anime/1-angelbeats/series/add',
			{'number': 1, 'product': 1, 'name': 'Name', 'length': 20, 'num_season': 1})
		self.assertEqual(response.status_code, 200)

		response = c.post('/anime/1-angelbeats/series/add',
			{'number': 1, 'product': 1, 'name': 'Name series', 'length': 20, 'num_season': 2})
		self.assertEqual(response.status_code, 200)

		response = c.get('/anime/1-angelbeats/series/list')
		self.assertIn('Name series', str(response.content, 'utf-8'))

	def test_add_serie_to_list(self):
		''' Добавление серии в список просмотренных '''
		request = RequestFactory().get('/anime/1-angelbeats/series/list')
		request.user = User.objects.get(username='first')
		request = Client().post('/mylist/series/add',
			{'product': 1, 'num_season': 1, 'number': 1})

		response = production_series_view()
		self.assertEqual(response.status_code, 200)
